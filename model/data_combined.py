...
class DataLoaderCombined( ):
    def __init__(self, X_path, Y_path, config, X_static_path):
        self._X_dynamic = np.load(X_path)
        self._Y = np.load(Y_path)
        self.max_length = config.max_length
        self.batch_size = config.batch_size

        static = np.load(X_static_path)
        self._X_static = np.tile(static, (1, self._X_dynamic.shape[1],1)).reshape(-1, self._X_dynamic.shape[1], 5)
        
        self._X = np.concatenate((self._X_dynamic, self._X_static), axis=2)
    ...
    def get_batch_train(self):
        ...
        Xs = self.X_train[ids, :, :31]
        Xstatic = self.X_train[ids, 0, -5:]
        ys = self.y_train[ids]
        return Xs, ys, Xstatic


    def get_batch_test(self):
        ids = np.random.randint( self.X_test.shape[0], size=self.batch_size)
        Xs = self.X_test[ids,:, :31]
        Xstatic = self.X_test[ids, 0, -5:]
        ys = self.y_test[ids]
        return Xs, ys, Xstatic
...
if __name__ == "__main__":
    ...
    d = DataLoaderCombined(X, Y, conf, X_static)
    x,y,s = d.get_batch_train()
    assert s.shape[1]==5
